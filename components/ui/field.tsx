...
function FieldError({
  ...
  const content = useMemo(() => {
    ...
    if (uniqueErrors?.length == 1) {
      return translateErrorMessage(uniqueErrors[0]?.message)
    }
    ...
    return (
      <ul className="ml-4 flex list-disc flex-col gap-1">
        {uniqueErrors.map(
          (error, index) =>
            error?.message && <li key={index}>{translateErrorMessage(error.message)}</li>
        )}
      </ul>
    )
  }, [children, errors, translateErrorMessage])
